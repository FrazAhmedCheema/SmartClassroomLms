...
const TeacherSettings = () => {
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:8080/teacher/change-password', {
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 bg-white text-gray-400 hover:text-[#1b68b3] focus:outline-none focus:ring-2 focus:ring-[#1b68b3]/40 p-1 rounded-full transition-colors shadow-md border border-gray-200"
                    style={{ boxShadow: '0 2px 8px 0 rgba(30, 64, 175, 0.08)' }}
                  >
                    {showPassword
                      ? <EyeOff size={22} strokeWidth={2.4} className="transition-transform duration-150 scale-110" />
                      : <Eye size={22} strokeWidth={2.4} className="transition-transform duration-150 scale-110" />}
                  </button>
                </div>
              </div>
              <div>
